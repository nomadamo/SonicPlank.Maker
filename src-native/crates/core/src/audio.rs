...
unsafe fn enumerate_audio_devices_mmdevapi() -> Result<Vec<AudioDeviceDef>> {
    use sonicplank_ipc::AudioDeviceKind;
...
pub fn start_audio_capture(
    ...
    if let Some(name) = device_id.strip_prefix("output:") {
        ...
        // CPAL input: peak emission not yet wired; drop the sender.
        drop(peak_tx);
        start_cpal_input(name)
    } else {
        // Legacy bare id: try loopback first, then input.
        start_wasapi_loopback(device_id, peak_tx)
...
unsafe fn wasapi_loopback_thread(
    ...
    loop {
        ...
        match capture_client.GetBuffer(&mut p_data, &mut num_frames, &mut flags, None, None) {
            ...
            Ok(()) => {
                // num_frames == 0: nothing ready yet — yield to avoid busy-spinning.
        ...
        // Emit live peak level every 100ms for the UI level meter.
