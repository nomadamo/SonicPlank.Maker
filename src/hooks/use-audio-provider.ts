...
const getResolvedDuration = (audioDuration: number): number => {
...
export function useAudioProvider({ tracks = [] }: { tracks?: Track[] } = {}) {
  ...
  const forceTimeUpdate = useCallback(() => {
    ...
    const currentTime = audio.currentTime;
    const duration = getResolvedDuration(audio.duration);
  ...
  useEffect(() => {
    ...
    const handleEnded = async () => {
      ...
      const audioDuration = getResolvedDuration(audioElement?.duration || 0);
    ...
    const handleCanPlay = () => {
      setState(getLoadingSuccessState(getResolvedDuration(audio.duration)));
    };
    ...
    const handleDurationChange = () => {
      setState({ duration: getResolvedDuration(audio.duration) });
    };
    ...
    const restoreState = async () => {
      ...
      const audioElement = htmlAudio.getAudioElement();
      const audioDuration = getResolvedDuration(audioElement?.duration || state.duration || 0);
      const isLiveStream = htmlAudio.isLive(audioDuration);
