...
export function VisualizerOverlayNode(NodeRef: NodeProps<FlowNodeType>) {
  ...
  const yVal = node.data.y !== undefined ? Number(node.data.y) : 70;
  const wVal = node.data.width !== undefined ? Number(node.data.width) : 80;
  ...
  const backgroundColor = (node.data.backgroundColor as string) || "rgba(0, 0, 0, 0.3)";
  const visualizerTypeVal = (node.data.visualizerType as string) || "bars";
      ...
      >
        ...
        <div className="flex flex-col gap-3 nodrag nopan nowheel">
          ...
          <div className="flex flex-col gap-1 border-t border-zinc-800/40 pt-2.5">
            <label className="text-[9px] font-semibold text-zinc-400 uppercase tracking-wider">Visualizer Style</label>
            <select
              value={visualizerTypeVal}
              onChange={(e) => handleUpdate({ visualizerType: e.target.value })}
          ...
          <div className="flex flex-col gap-1 border-t border-zinc-800/40 pt-2.5">
            <label className="text-[9px] font-semibold text-zinc-400 uppercase tracking-wider">Background Color</label>
