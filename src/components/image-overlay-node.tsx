...
function fromNodeData(data: FlowNodeType["data"]) {
  return {
    ...
    imagePath: (data.imagePath as string) ?? DEFAULTS.imagePath,
  };
}
      ...
      >
        <div className="flex flex-col gap-3 nodrag nopan nowheel">
          ...
          <div className="flex flex-col gap-2 border-t border-border/40 pt-2.5">
            <label className="text-[9px] font-semibold text-muted-foreground uppercase tracking-wider">Image File</label>
            <div className="flex gap-1.5">
              <input
                type="text"
                readOnly
                value={draft.imagePath}
                className="flex-1 bg-muted border border-border rounded px-2.5 py-1.5 text-xs text-muted-foreground select-all overflow-hidden text-ellipsis whitespace-nowrap text-left"
                placeholder="Select image file..."
                title={draft.imagePath}
              />
              <button
                onClick={handleSelectImage}
                className="px-3 py-1.5 bg-secondary border border-border/80 hover:bg-secondary/80 text-foreground/80 text-xs font-medium rounded cursor-pointer transition-colors"
              >
                Browse
              </button>
            </div>
