...
import { UnknownIcon } from "@fluentui/react-icons-mdl2";
import { AccordionHeader } from "@base-ui/react";
...
const RTMP_PRESETS = [
  ...
  { id: "youtube", label: "YouTube", url: "rtmp://a.rtmp.youtube.com/live2/" },
  {
    id: "tiktok",
    label: "TikTok",
    url: "rtmp://push-rtmp-f5-va.tiktokcdn.com/",
  },
...
function YoutubeUploadSection({
  ...
  const isConnected = !!settings.youtubeRefreshToken;
  ...
  const handleConnect = async () => {
    ...
    try {
      const { refreshToken } = await window.electron.initiateYoutubeAuth({
        clientId,
        clientSecret,
      });
      updateSettings({ youtubeRefreshToken: refreshToken });
    } catch (err) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          <Label className="text-xs font-medium flex items-center gap-1.5">
            <IconBrandYoutube className="w-3 h-3 text-red-500" />
            Auto-upload to YouTube
          </Label>
          <span className="text-xs text-muted-foreground">
            Upload the recording to YouTube as a private video after the stream
            ends
          </span>
        </div>
        {isConnected && (
          <Switch
            checked={!!settings.youtubeAutoUpload}
            onCheckedChange={(v) => updateSettings({ youtubeAutoUpload: v })}
          />
        )}
      </div>
      ...
      {isConnected ? (
        <div className="flex items-center justify-between gap-2 text-xs">
          <span className="text-green-500 flex items-center gap-1">
            <Check className="w-3 h-3" /> YouTube connected
          </span>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="flex flex-col gap-2">
          <Input
            ...
            onChange={(e) =>
              updateSettings({ youtubeClientId: e.target.value.trim() })
            }
          ...
          <Input
            ...
            onChange={(e) =>
              updateSettings({ youtubeClientSecret: e.target.value.trim() })
            }
            className="text-xs font-mono"
          />
          <div className="flex items-center gap-2">
            ...
            >
              {isConnecting ? "Opening browser…" : "Connect YouTube"}
            </Button>
          </div>
          {connectError && (
          ...
          <span className="text-xs text-muted-foreground leading-relaxed">
            Requires a Google Cloud OAuth 2.0 Desktop client with redirect URI{" "}
            <code className="bg-muted px-1 py-0.5 rounded text-[10px]">
              http://localhost:8079/oauth/callback
            </code>
          </span>
        </div>
      )}
...
export function OutputSettingsTab() {
  ...
  React.useEffect(() => {
    window.electron.getGpuList().then((list) => {
      ...
      if (
        list &&
        list.length === 1 &&
        (!settings.selectedGpu || settings.selectedGpu === "auto")
      ) {
  ...
  const updateTarget = (id: string, patch: Partial<RtmpTarget>) => {
    updateSettings((prev) => ({
      rtmpTargets: (prev.rtmpTargets || []).map((t) =>
        t.id === id ? { ...t, ...patch } : t,
      ),
    }));
  };
  ...
  const getServiceIcon = (preset?: string) => {
    switch (preset) {
      case "youtube":
  ...
  return (
    <TabsContent value="output" className="flex flex-col min-h-[360px] pr-2">
      <Accordion defaultValue={["global"]} className="border-none w-full">
        {/* FFmpeg Global Settings */}
        <AccordionItem
          value="global"
          className="border-none bg-muted/20 rounded-lg overflow-hidden"
        >
          <AccordionTrigger className="px-4 py-3 hover:bg-secondary/50 hover:no-underline border-none">
            <h3 className="text-sm m-0">Global Encoder Settings</h3>
          </AccordionTrigger>
          <AccordionContent className="p-4 flex flex-col border-none gap-4">
            <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3">
              <Button
                variant="outline"
                size="sm"
                onClick={() => setEncoderFromPreset("record")}
              >
                Apply "Record" Preset
              </Button>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setEncoderFromPreset("stream")}
              >
                Apply "Stream" Preset
              </Button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSettings({ streamEncoder: v as any })
                  }
                >
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSettings({
                      streamFps: parseInt(v) as 30 | 60 | 90 | 120,
                    })
                  }
                >
                  <SelectTrigger className="text-xs">
              ...
              <div className="flex flex-col gap-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    updateSettings({ streamOutputResolution: v })
                  }
                >
                  <SelectTrigger className="text-xs">
                    <SelectValue />
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="936p">
                      936p (1664×936) — Twitch Sweet Spot
                    </SelectItem>
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateSettings({
                      streamBitrateKbps: parseInt(e.target.value) || 6000,
                    })
                  }
                  className="text-xs font-mono"
                />
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateSettings({
                      streamDelayMs: parseInt(e.target.value) || 0,
                    })
                  }
                  className="text-xs font-mono"
                />
        ...
        >
          <AccordionTrigger className="px-4 py-3 hover:bg-secondary/50 hover:no-underline border-none">
            <h3 className="text-sm m-0">Recording Settings</h3>
          </AccordionTrigger>
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                value={
                  ...
                  "Default (Documents/SonicPlank.Maker/recordings)"
                }
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    console.error(
                      "Failed to pick recording path directory:",
                      err,
                    );
                  }
                }}
            ...
            <div className="flex flex-col gap-1.5 text-left mt-1">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  updateSettings({
                    recordingBitrateKbps: isNaN(val) ? undefined : val,
                  });
                }}
            ...
            <div className="flex items-center justify-between gap-3 pt-1">
              <div className="flex flex-col gap-0.5">
                <Label className="text-xs font-medium">
                  Record stream (stream copy)
                </Label>
                <span className="text-xs text-muted-foreground">
                  Save a local MP4 of the stream alongside RTMP output
                </span>
              </div>
              <Switch
                checked={!!settings.recordStreamEnabled}
                onCheckedChange={(v) =>
                  updateSettings({ recordStreamEnabled: v })
                }
              />
            </div>

            {settings.recordStreamEnabled && (
              <div className="flex flex-col gap-3 pl-3 border-l-2 border-border/50">
        ...
        >
          <AccordionTrigger className="px-4 py-3 hover:bg-secondary/50 hover:no-underline border-none">
            <h3 className="text-sm m-0">Stream Services</h3>
          </AccordionTrigger>
            ...
            <div className="flex flex-col gap-4">
              {(settings.rtmpTargets || []).length === 0 ? (
                <p className="text-xs text-muted-foreground italic">
                  No stream services configured.
                </p>
              ) : (
                settings.rtmpTargets.map((target) => (
                  <div
                    key={target.id}
                    className="flex flex-col gap-3 p-3 bg-muted/50 border border-border rounded-lg"
                  >
                    <div className="flex items-center justify-between gap-4">
                      {getServiceIcon(target.preset)}
                      <Input
                        value={target.label}
                        onChange={(e) =>
                    ...
                    <div className="grid grid-cols-[100px_1fr] items-center gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v: any) => {
                          const presetUrl =
                            RTMP_PRESETS.find((p) => p.id === v)?.url || "";
